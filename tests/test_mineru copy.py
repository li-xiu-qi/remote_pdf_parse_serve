...
if __name__ == '__main__':
    ...
    pdf_files_dir = os.path.join(__dir__, "pdfs")  # PDF文件夹路径
    output_dir = os.path.join(__dir__, "output")   # 输出结果文件夹路径
    pdf_suffixes = [".pdf"]  # 支持的PDF后缀
    image_suffixes = [".png", ".jpeg", ".jpg"]  # 支持的图片后缀

    doc_path_list = ["/home/xiaoke/projects/remote_pdf_parse_serve/assets/pdfs/simcse.pdf"]  # 文档路径列表
    ...
    """如果环境不支持VLM，建议使用pipeline模式"""
    # res = parse_doc(doc_path_list, output_dir, backend="pipeline")
    

    """如需启用VLM模式，将backend改为'vlm-xxx'"""
    # parse_doc(doc_path_list, output_dir, backend="vlm-transformers")  # 通用。
    res = parse_doc(doc_path_list, output_dir, backend="vlm-sglang-engine")  # 更快(engine)。
